 ...
 * Adapted for Node.js CommonJS by removing all browser-specific APIs.
 *
 * Licensed under the GPL
 *
 * Public API:
 *   loadFromRGBA(rgbaData, width, height) — load bitmap from raw RGBA pixel data
 *   setParameter({...})                   — set tracing parameters
 *   process()                             — run the Potrace algorithm (synchronous)
 *   getSVG(size, opt_type)                — return SVG string of traced result
...
// ── Public: load from raw RGBA pixel data ───────────────────────────────────

/**
 * Build a binary Bitmap from raw RGBA pixel data (e.g., from @resvg/resvg-js).
 * Alpha-premultiplication is applied before the luminance threshold so that
 * fully-transparent pixels are treated as white (background).
 *
 * @param {Uint8ClampedArray|Buffer} rgbaData  - RGBA bytes, 4 bytes per pixel
 * @param {number} width
 * @param {number} height
 */
function loadFromRGBA(rgbaData, width, height) {
    if (info.isReady) {
        clear();
    }
    ...
    for (var i = 0; i < l; i += 4) {
        ...
        var g = rgbaData[i + 1];
        var b = rgbaData[i + 2];
        var a = rgbaData[i + 3] / 255;
        // Premultiply alpha: transparent pixels collapse to white background
        var rp = r * a + 255 * (1 - a);
        var gp = g * a + 255 * (1 - a);
        var bp = b * a + 255 * (1 - a);
        var lum = 0.2126 * rp + 0.7153 * gp + 0.0721 * bp;
        bm.data[j] = (lum < 128 ? 1 : 0);
        j++;
    }
    info.isReady = true;
}
...
// ── Public: process (synchronous) ──────────────────────────────────────────

/**
 * Run the Potrace algorithm synchronously.
 * loadFromRGBA() must be called first.
 */
function process() {
    if (!info.isReady) {
        throw new Error('potrace: bitmap not loaded — call loadFromRGBA() first');
    }
    bmToPathlist();
    processPath();
}
...
/**
 * Return an SVG string of the traced result.
 * @param {number} size       — scale factor (1 = original pixel coordinates)
 * @param {string} [opt_type] — pass "curve" to render as stroked outlines
 * @returns {string}
 */
function getSVG(size, opt_type) {

    function pathToD(curve) {

        function bezierSeg(i) {
...
/**
 * Reset internal state so the module can be reused for a new image.
 */
function clear() {
    bm = null;
    pathlist = [];
